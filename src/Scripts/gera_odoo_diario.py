...
API_SLEEP_BETWEEN_REQUESTS = 0.05
API_SLEEP_BETWEEN_BATCHES = 1.0
API_TIMEOUT = 30
API_MAX_RETRIES = 3
API_RETRY_SLEEP = 2.0
...
def fetch_vendor_bill_ids_after(last_id=0, limit=100):
    """
    Busca vendor bills alteradas ontem, incluindo canceladas.
    """
    env.cr.execute("""
        SELECT id
        FROM account_move
        ...
        ORDER BY id ASC
        LIMIT %s
    """, [START_UTC, END_UTC, last_id, limit])
...
def map_invoice_status(inv):
    if inv.state == "cancel":
        return "cancelled"
    if inv.state == "posted":
        return "posted"
    return inv.state or "draft"

def build_vendor_bill_payloads(inv):
    """
    Se cancelada:
      - ainda envia o documento
      - status = cancelled
      - mantém items por projeto para localizar o documento correto na base
    """
    grouped = {}
    ...
    for line in inv.invoice_line_ids:
        ...
        # para canceladas, mantém item mesmo se subtotal zerado
        if inv.state != "cancel" and subtotal <= 0:
            continue
    ...
    # fallback: se a cancelada não tiver item/projeto resolvido, tenta cabeçalho
    if not grouped and inv.state == "cancel":
        header_project = False
        if hasattr(inv, "project_id") and inv.project_id:
            header_project = inv.project_id

        if header_project:
            grouped[str(header_project.id)] = {
                "projectId": str(header_project.id),
                "projectCode": normalize_spaces(header_project.name or ""),
                "items": [],
                "amountTotal": abs_float(inv.amount_total),
            }
    ...
    for project_id, data in grouped.items():
        payloads.append({
            ...
            "status": map_invoice_status(inv),
            "type": "vendor_bill",
            "sourceDocument": inv.invoice_origin or "",
            "syncDate": iso_now(),
            "updatedAt": iso_now(),
            "paymentStatus": inv.payment_state or "",
            "isCancelled": inv.state == "cancel",
            "vendor": {
...
def fetch_svl_ids_after(last_id=0, limit=200):
    """
    Busca valuation layers alteradas ontem.
    """
    env.cr.execute("""
        SELECT id
...
print("🚀 Incremental diário - ontem")
print("   Vendor Bills + cancelamentos + Stock/Returns")
...
while True:
    try:
        env.cr.rollback()
        vendor_ids = fetch_vendor_bill_ids_after(last_id=last_vendor_id, limit=VENDOR_BATCH_SIZE)
                ...
                for payload in payloads:
                    ...
                    if ok:
                        if payload["status"] == "cancelled":
                            cancelled_vendor += 1
                            print("🟠 Bill cancelada sincronizada:", payload["invoiceNumber"], "| projeto:", payload["projectCode"])
                        else:
                            sent_vendor += 1
                            print("✅ Vendor Bill sincronizada:", payload["invoiceNumber"], "| projeto:", payload["projectCode"])
                    else:
                        errors += 1
        ...
        print(
            "📊 Lote Vendor %s | ultimo_id=%s | enviadas=%s | canceladas=%s | ignoradas=%s | erros=%s"
            % (vendor_batch_num, last_vendor_id, sent_vendor, cancelled_vendor, skipped_vendor, errors)
        )
...
print("✅ Vendor Bills finalizadas | enviadas=%s | canceladas=%s | ignoradas=%s | erros=%s" % (
    sent_vendor, cancelled_vendor, skipped_vendor, errors
))
            ...
            try:
                ...
                if ok:
                    if payload["type"] == "stock_return":
                        sent_return += 1
                        print("✅ Stock Return sincronizada:", payload["invoiceNumber"], "| projeto:", payload["projectCode"])
                    else:
                        sent_stock += 1
                        print("✅ Stock Out sincronizada:", payload["invoiceNumber"], "| projeto:", payload["projectCode"])
                else:
                    errors += 1
...
print("🏁 FINALIZADO")
print(
    "Resumo -> Vendor Bills=%s | Vendor Canceladas=%s | Stock Out=%s | Stock Return=%s | Ignorados Vendor=%s | Ignorados Stock=%s | Ignorados Return=%s | Erros=%s"
    % (sent_vendor, cancelled_vendor, sent_stock, sent_return, skipped_vendor, skipped_stock, skipped_return, errors)
)