...
# Filtra somente estes dois projetos para teste
TARGET_PROJECT_KEYS = {    "353",
    "421",
    "420",
    "422",
    "16844 SW 41ST AVENUE RD",
    "1902 W ADELIA PLACE",
    "1948 W ADELIA PLACE",
    "1932 W ADELIA PLACE",
    "3862 SW 147ST",
...
def project_matches_target(project):
    if not project:
        return False

    values = [
        str(getattr(project, "id", "") or ""),
        normalize_spaces(getattr(project, "name", "") or ""),
        normalize_spaces(getattr(project, "display_name", "") or ""),
    ]

    for v in values:
        upper_v = v.upper()
        for key in TARGET_PROJECT_KEYS:
            key_up = str(key).upper()
            if upper_v == key_up:
                return True
            if key_up in upper_v:
                return True
...
def build_vendor_bill_payloads(inv):
    ...
    for line in inv.invoice_line_ids:
        ...
        if not project_matches_target(line_project):
            continue
...
def build_svl_payload(svl):
    ...
    if not project_matches_target(project):
        return None, "fora do filtro dos 2 projetos"
...
print("🚀 Sincronização de teste somente para os projetos:", ", ".join(sorted(TARGET_PROJECT_KEYS)))
