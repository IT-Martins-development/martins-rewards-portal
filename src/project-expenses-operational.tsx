...
function getBudgetedValue(row: AnyObj) {
  return round2(
    row?.budgetedTotal ??
      row?.budgetedCost ??
      row?.totalBudget ??
...
function parseDetailData(payload: DetailApiResponse | AnyObj): DetailData | null {
  ...
  const parsedGroups = groups.map((group: AnyObj) => ({
  ...
  const fallbackBudgetFromGroups = parsedGroups.reduce((acc: any, group: { totalBudget: any; }) => acc + group.totalBudget, 0);
  ...
  return {
    ...
    budgetedTotal: round2(
      data?.budgetedTotal ??
        data?.budgetedCost ??
        data?.totalBudget ??
        fallbackBudgetFromGroups
    ),
    ...
    totals: {
      ...
      totalBudget: round2(data?.totals?.totalBudget ?? fallbackBudgetFromGroups),
...
export default function ProjectExpensesOperational() {
  ...
  const totalPages = useMemo(
    () => Math.max(1, Math.ceil(rows.length / filters.pageSize)),
  ...
  const totals = useMemo(() => {
    return rows.reduce(
      (acc, row) => {
        acc.budgetedTotal += row.budgetedTotal;
    ...
    <div style={S.page}>
      <div style={S.header}>
        <div style={S.titleWrap}>
          <h2 style={S.title}>Project Expenses Operacional</h2>
          <p style={S.subtitle}>
            Consulte despesas operacionais por projeto, compare com o budgeted e veja os detalhes
      ...
      <div style={S.cardsGrid}>
        ...
        <SummaryCard title="Budgeted" value={formatMoney(totals.budgetedTotal)} subtitle="Sum of budgeted values" />
              ...
              paginatedRows.map((row, index) => {
                ...
                return (
                  <tr key={`${row.projectId || row.projectName}-${index}`}>
                    ...
                    <td style={S.td}>{formatMoney(row.budgetedTotal)}</td>
              ...
              <div style={{ display: "grid", gap: 6 }}>
                ...
                {detailModal.data ? (
                  <div style={S.chipRow}>
                    ...
                    <span style={S.chip}>Budgeted: {formatMoney(filteredDetailTotals.totalBudget)}</span>
